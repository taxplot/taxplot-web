import { VictoryChart, VictoryLine, VictoryScatter, VictoryAxis, VictoryArea } from 'victory'
import { rates1861, rates1862, rates1862fake, rates2021single, deduction1861, deduction1862, deduction2021single } from '../../../../data/tax-brackets-data'
...
const TestPlot = ({slideIndex}) => {
    ...
    useEffect(() => {
        if (slideIndex <= 0) { 
            ...
            setTaxPayer({...taxPayer, 
                year: 2021,
                deduction: deduction2021single
            })
            setTickValuesX(null)
            setTickValuesY(null)
            setLabelNames(["Marginal Tax Rate", "Effective Tax Rate", "32% Tax Bracket"])
            setLabelPoints([{x:770000, y:0.35}, {x:600000, y:0.25}, {x:420000, y:.1}])
            setShowDollar(false)
            setBracketArea([
                {x:164925+deduction2021single, y:.32},
                {x:209425+deduction2021single, y:.32}
            ])
        ...
        else if(slideIndex === 2) { //dollar slices 1861
            setBrackets(rates1861)
            setDeduction(deduction1861)
            setShowEffectiveArea(false)
            setRangeMultiple(10)
            ...
            setLabelPoints([{x:400, y:0.1}, {x:1200, y:.1}, {x:1200, y:.007}])
            setShowDollar(true)
            setGreenArea([
                {x:200, y:0, y0:0},
                {x:200, y:1, y0:0},
            ...
            setLabelPoints([{x:1600, y:0.03}, {x:2000, y:.012}])
            setShowDollar(false)
            setBracketArea(null)
            ...
            setTaxPayer({...taxPayer, 
                year: 1862,
                deduction: deduction1862
            })
            setTickValuesX([600, 10000])
            setTickValuesY([.01, .03, .05, .07])
            setLabelNames(["Marginal Tax Rate"])
        ...
        else if(slideIndex === 5) { //dollar slices 1862
            ...
            setLabelPoints([{x:300, y:0.1}, {x:900, y:.1}, {x:900, y:.005}, {x:1500, y:.1}, {x:1500, y:.01}])
            setShowDollar(true)
            setGreenArea([
                {x:100, y:0, y0:0},
                {x:100, y:1, y0:0},
            ...
            setLabelPoints([{x:5000, y:0.03}, {x:6000, y:.02}, {x:10500, y: .025}])
            setShowDollar(false)
            setBracketArea(null)
            ...
            setTickValuesY(null)
            setLabelNames(["Marginal Tax Rate", "Effective Tax Rate"])
            setLabelPoints([{x:770000, y:0.35}, {x:600000, y:0.25}])
            setShowDollar(false)
            setBracketArea(null)
    ...
    const maxPercent = bracketPlot(taxBrackets,deduction, xMax)[bracketPlot(taxBrackets,0).length - 1].y * rangeMultiple 
    const tickFormat = (t) => {
